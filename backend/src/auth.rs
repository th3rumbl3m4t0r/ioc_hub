...
pub struct Session {
    pub username: String,
    pub ip: String,
    /// TLS JA3 fingerprint captured at login (when the edge supplies one).
    /// Used as a fallback when the source IP changes — see `authorize`.
...
impl Auth {
    ...
    pub fn issue(&mut self, username: &str, ip: &str, ja3: Option<String>) -> String {
        let token = random_token();
        self.tokens.insert(
            token.clone(),
            Session { username: username.to_string(), ip: ip.to_string(), ja3, created: now() },
        );
        token
    }
    ...
    /// Resolve a token to a username, enforcing TTL and source binding.
    ///
    /// Binding logic (the JA3 fallback exists for egress proxies like Zscaler,
    /// whose source IP rotates across a changing pool of addresses but whose TLS
    /// fingerprint is highly standardised):
    ///   1. If the request comes from the SAME source IP the token was issued
    ///      to → allow (the fast, strict path).
    ///   2. Otherwise, IF a JA3 fingerprint was captured at login AND the request
    ///      carries a matching JA3 → allow. This lets a session survive an IP
    ///      change as long as the TLS fingerprint is identical.
    ///   3. Otherwise → reject (without deleting the session, so the legitimate
    ///      origin keeps working).
    ///
    /// `req_ja3` is None when JA3 fallback is disabled or the edge supplied no
    /// fingerprint; in that case this is exactly the previous pure-IP binding.
    pub fn authorize(&mut self, token: &str, ip: &str, req_ja3: Option<&str>) -> Option<String> {
        match self.tokens.get(token) {
            Some(s) => {
                ...
                // 1) same source IP → always allowed
                if s.ip == ip {
                    return Some(s.username.clone());
                }
                // 2) IP changed — fall back to JA3 if we have one on both sides
                if let (Some(sess_ja3), Some(req)) = (s.ja3.as_deref(), req_ja3) {
                    if !sess_ja3.is_empty() && sess_ja3 == req {
                        return Some(s.username.clone());
                    }
                }
                // 3) neither IP nor JA3 matched
                None
            }
            None => None,
        }
