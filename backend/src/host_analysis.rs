...
pub fn analyze_domain(domain: &str) -> Value {
    ...
    if tool_exists("openssl") {
        if let Some(cert) = grab_tls_cert(domain) {
...
///
/// SECURITY: the domain is passed ONLY as argv to `openssl` (-connect/-servername)
...
/// structurally impossible regardless of what `domain` contains, independent of
/// the upstream `valid_domain` check (which still runs, as defense in depth).
fn grab_tls_cert(domain: &str) -> Option<Value> {
    let to = Duration::from_secs(30);
    let connect = format!("{domain}:443");
    let txt = run_piped(
        ("openssl", &["s_client", "-connect", &connect, "-servername", domain]),
...
/// Analyze an IP: whois (netblock/org/ASN) plus an nmap full-port scan that runs
/// the ssl-cert script (`nmap -Pn --script ssl-cert -p-`), surfacing open ports
/// and any TLS certificate nmap finds. Returns `{ attributes: {...} }`.
pub fn analyze_ip(ip: &str) -> Value {
    ...
    // ---- nmap full-port scan + ssl-cert script ----
    if !tool_exists("nmap") {
        // whois may still have produced something useful; only error if we have nothing.
        if out.len() <= 1 {
            return json!({"error": "nmap is not installed on the server"});
        }
        out.insert("_source".into(), json!("host-analysis"));
        return json!({ "attributes": Value::Object(out) });
    }
    // -Pn (skip host discovery), --script ssl-cert, -p- (all ports), --open
    // (trim closed). -oG - emits greppable output for the port parsing; the
    // ssl-cert script output is appended as "Script:" host comments we also read.
    let v6 = if ip.contains(':') { vec!["-6"] } else { vec![] };
    let mut args: Vec<&str> = vec!["-Pn", "--script", "ssl-cert", "-p-", "--open", "-oG", "-"];
    args.extend(v6);
    args.push(ip);
    ...
    // Greppable format: a "Ports:" line holds "22/open/tcp//ssh//...,443/open/..."
    // and (with NSE) a "Script:" / "Ignored" section may carry ssl-cert output.
    for line in txt.lines() {
        if let Some(idx) = line.find("Ports:") {
            let portsec = &line[idx + 6..];
            // the greppable line can also contain a trailing "Ignored State:"; cut it
            let portsec = portsec.split("Ignored").next().unwrap_or(portsec);
            for entry in portsec.split(',') {
                let e = entry.trim();
                let f: Vec<&str> = e.split('/').collect();
                if f.len() >= 5 && f[1] == "open" {
                    if let Ok(p) = f[0].parse::<u32>() {
                        if !ports.contains(&p) { ports.push(p); }
                        let svc = f.get(4).map(|s| s.trim()).unwrap_or("");
                        let ver = f.get(6).map(|s| s.replace('|', " ").trim().to_string()).unwrap_or_default();
                        let mut s = Map::new();
                        s.insert("port".into(), json!(p));
                        s.insert("protocol".into(), json!(f.get(2).copied().unwrap_or("tcp")));
                        if !svc.is_empty() { s.insert("service".into(), json!(svc)); }
                        if !ver.is_empty() { s.insert("product".into(), json!(ver)); }
                        services.push(Value::Object(s));
                    }
                }
            }
        }
        // In greppable output NSE script results ride after a "Script:" tag or as
        // "\t" comment lines; capture anything mentioning the cert.
        if let Some(idx) = line.find("ssl-cert") {
            let frag = line[idx..].trim().to_string();
            if !frag.is_empty() { cert_lines.push(frag); }
        }
    }
    ...
    if !cert_lines.is_empty() {
        let blob = cert_lines.join("\n");
        out.insert("ssl_cert".into(), json!(blob));
        if let Some(s) = whois_field(&blob, &["Subject:"]) { out.insert("ssl_cert_subject".into(), json!(s)); }
        if let Some(s) = whois_field(&blob, &["Issuer:"]) { out.insert("ssl_cert_issuer".into(), json!(s)); }
        if let Some(s) = whois_field(&blob, &["SHA-1:", "SHA1:"]) { out.insert("ssl_cert_sha1".into(), json!(s)); }
    }
    out.insert("_source".into(), json!("host-analysis"));
