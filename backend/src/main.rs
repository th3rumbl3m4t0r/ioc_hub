...
use http_util::{client_ip, header, json_string, read_body, send_json, Json};
...
fn require_auth(req: &Request, state: &AppState) -> Result<String, Json> {
    ...
    a.authorize(&token, &ip).ok_or_else(|| Json::err(401, "invalid token or source IP changed"))
}
...
fn h_sso_login(mut req: Request, state: &AppState) {
    ...
    let token = state.auth.lock().unwrap().issue(&username, &ip);
