...
/* ============================ PNG / image export ======================= */
// Render the whole graph to a PNG data URL, fit to the furthest entities with a
// ~50px margin, at high resolution (no fixed cap — scale is chosen so even a
// big graph stays legible). Respects the current accent (already baked into the
// node/edge styles) and the active day/night background.
function graphPngDataUrl(opts = {}) {
  if (!state.cy) return null;
  const bg = opts.transparent ? undefined : cssVar('--graph-bg');
  // Pick a scale that yields a high-detail image. Cytoscape `full:true` fits the
  // graph's bounding box; we aim for a large output but clamp the scale so we
  // don't ask the canvas for something absurd on huge graphs (the browser caps
  // canvas dimensions ~16k px; we stay well under by lowering scale as the graph
  // grows, while keeping small graphs crisp).
  const bb = state.cy.elements().boundingBox();
  const w = Math.max(1, bb.w), h = Math.max(1, bb.h);
  const longest = Math.max(w, h);
  // target ~the longest side rendered at this many device px, then derive scale
  const TARGET = 4000;
  let scale = TARGET / longest;
  scale = Math.max(1, Math.min(scale, 4));           // 1x..4x
  // keep the largest dimension under a safe canvas limit (~15000 px)
  const SAFE = 15000;
  if (longest * scale + 100 > SAFE) scale = (SAFE - 100) / longest;
  return state.cy.png({
    output: 'base64uri',
    full: true,           // whole graph, not just viewport
    bg,                   // theme background (omit for transparent)
    scale,
    maxWidth: SAFE,
    maxHeight: SAFE,
    // Cytoscape adds this many *rendered* px of padding around the content.
    // ~50px from the furthest entities, as requested.
    padding: 50,
  });
}

function exportGraphPng() {
  if (!state.cy || !Object.keys(state.G.entities).length) { toast('nothing to export', true); return; }
  const tk = startTask('export-png');
  try {
    const uri = graphPngDataUrl();
    if (!uri) { tk.fail('no graph'); return; }
    const a = el('a', { href: uri, download: `iochub-graph-${new Date().toISOString().slice(0, 10)}.png` });
    document.body.appendChild(a); a.click(); a.remove();
    tk.done('saved');
    toast('graph exported as PNG');
  } catch (e) { tk.fail(e.message); toast('PNG export failed: ' + e.message, true); }
}

/* ============================ PDF export =============================== */
// Build the structured data the PDF needs: per-entity records (the same
// indicator info the MISP export emits) plus each entity's connections, and a
// flat connection list. Kept here (not in the heavy module) so the module stays
// purely about rendering.
function buildPdfModel() {
  const ents = Object.values(state.G.entities);
  const subs = state.G.subs || {};
  const FILE_HASHES = { md5: 'md5', sha1: 'sha1', sha256: 'sha256', sha512: 'sha512',
    ssdeep: 'ssdeep', tlsh: 'tlsh', imphash: 'imphash', vhash: 'vhash',
    authentihash: 'authentihash', telfhash: 'telfhash', permhash: 'permhash' };

  // label helper for any node id (entity or sub)
  const nodeLabel = (id) => {
    const e = state.G.entities[id];
    if (e) return { kind: e.kind, value: e.value, name: e.name && e.name !== e.value ? e.name : null };
    const s = subs[id];
    if (s) return { kind: 'sub', value: s.value, name: s.field };
    return { kind: '?', value: id, name: null };
  };

  // per-entity indicator records (MISP-equivalent), restricted to meaningful data
  const entityRecords = ents.map(e => {
    const a = e.attributes || {};
    const indicators = [];        // [{label, value}] the "MISP info"
    if (e.kind === 'file') {
      const ev = String(e.value);
      const evType = ev.length === 32 ? 'md5' : ev.length === 40 ? 'sha1' : ev.length === 128 ? 'sha512' : 'sha256';
      const seen = new Set();
      const push = (label, val) => { if (val && !seen.has(label + ':' + val)) { seen.add(label + ':' + val); indicators.push({ label, value: String(val) }); } };
      push(evType, ev);
      Object.entries(FILE_HASHES).forEach(([k, type]) => { if (a[k]) push(type, a[k]); });
      const names = new Set();
      if (a.meaningful_name) names.add(String(a.meaningful_name));
      Object.keys(a).filter(k => /^names(\[\d+\])?$/.test(k)).forEach(k => { if (a[k]) names.add(String(a[k])); });
      if (a['_local.filename']) names.add(String(a['_local.filename']));
      [...names].forEach(n => push('filename', n));
    } else if (e.kind === 'domain') {
      indicators.push({ label: 'domain', value: String(e.value) });
    } else if (e.kind === 'ip') {
      indicators.push({ label: 'ip-dst', value: String(e.value) });
    } else if (e.kind === 'url') {
      indicators.push({ label: 'url', value: String(e.value) });
    }
    // connections of this entity (edges touching it), with the far-side label
    const connections = [];
    Object.values(state.G.edges).forEach(ed => {
      let other = null;
      if (ed.source === e.id) other = ed.target;
      ...
      if (other) {
        const ol = nodeLabel(other);
        connections.push({ relation: ed.label || '', toKind: ol.kind, to: ol.value, toName: ol.name });
      }
    });
    return {
      kind: e.kind,
      value: e.value,
      name: e.name && e.name !== e.value ? e.name : null,
  ...
  // flat connection list (each undirected edge once)
  const connList = Object.values(state.G.edges).map(ed => {
    const s = nodeLabel(ed.source), t = nodeLabel(ed.target);
    return { from: s.value, fromKind: s.kind, relation: ed.label || '', to: t.value, toKind: t.kind };
  });
  ...
  return {
    ...
    counts: {
      ...
      files: ents.filter(e => e.kind === 'file').length,
      domains: ents.filter(e => e.kind === 'domain').length,
      ips: ents.filter(e => e.kind === 'ip').length,
      urls: ents.filter(e => e.kind === 'url').length,
...
// Inject the heavy PDF module (jsPDF + renderer) once, then tear nothing down
// permanently (jsPDF is reusable); we just lazy-load on first use.
let _pdfModuleLoading = null;
function loadPdfModule() {
  if (window.IoCHubPdf && window.jspdf) return Promise.resolve();
  if (_pdfModuleLoading) return _pdfModuleLoading;
  _pdfModuleLoading = new Promise((resolve, reject) => {
    ...
    // jsPDF, then the renderer module (no external font/logo assets)
    load('vendor/jspdf.umd.min.js')
      .then(() => load('pdf-export.js'))
      .then(() => resolve())
      .catch(reject);
  });
...
async function exportGraphPdf() {
  if (!state.cy || !Object.keys(state.G.entities).length) { toast('nothing to export', true); return; }
  const tk = startTask('export-pdf', 'building…');
  try {
    busy(true, 'building PDF…');
    await loadPdfModule();
    if (!window.IoCHubPdf || typeof window.IoCHubPdf.build !== 'function') {
      throw new Error('PDF module failed to load');
    }
    const model = buildPdfModel();
    const graphPng = graphPngDataUrl({ transparent: false });
    const accent = cssVar('--accent');
    const filename = `iochub-report-${new Date().toISOString().slice(0, 10)}.pdf`;
    window.IoCHubPdf.build({ model, graphPng, accent, filename });
    tk.done('saved');
...
function wire() {
  ...
  $('#btnExportPdf').addEventListener('click', exportGraphPdf);
